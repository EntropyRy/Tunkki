...
final class MemberPasswordFormTest extends FixturesWebTestCase
{
    ...
    public function testPasswordFormRendersRepeatedFields(): void
    {
        ...
        if ($status < 200 || $status >= 300) {
            ...
            @fwrite(
                \STDERR,
                '[MemberPasswordFormTest] GET /en/profile/password status='.
                    $status.
                    ' Location='.
                    $loc.
                    \PHP_EOL,
            ...
            try {
                ...
                $roles =
                    $tok && method_exists($tok, 'getRoleNames')
                        ? implode(',', $tok->getRoleNames())
                        : '';
                @fwrite(
                    \STDERR,
                    '[MemberPasswordFormTest] token='.
                        ($tok ? $tok::class : 'null').
                        ' userType='.
                        (\is_object($u) ? $u::class : \gettype($u)).
                        ' roles='.
                        $roles.
                        \PHP_EOL,
                );
                ...
                @fwrite(
                    \STDERR,
                    '[MemberPasswordFormTest] token diag failed: '.
                        $e->getMessage().
                        \PHP_EOL,
                );
            }
        ...
        // If redirected to login once, follow Location, seed home, and retry
        if (\in_array($status, [301, 302, 303], true)) {
            $loc = $this->client->getResponse()->headers->get('Location') ?? '';
            $path = parse_url($loc, \PHP_URL_PATH) ?: $loc;
            if (
                '' !== $path
                && (str_contains($path, '/en/login')
        ...
        $this->assertSame(
            200,
            $status,
            'Password form page should load (200).',
        );
        ...
        $this->assertGreaterThan(
            0,
            $crawler
                ->filter('input[name="user_password[plainPassword][first]"]')
                ->count(),
        ...
        $this->assertGreaterThan(
            0,
            $crawler
                ->filter('input[name="user_password[plainPassword][second]"]')
                ->count(),
    ...
    public function testPasswordFormRejectsMismatchedPasswords(): void
    {
        ...
        if ($status < 200 || $status >= 300) {
            ...
            @fwrite(
                \STDERR,
                '[MemberPasswordFormTest] GET /en/profile/password (mismatch) status='.
                    $status.
                    ' Location='.
                    $loc.
                    \PHP_EOL,
        ...
        $this->assertGreaterThan(
            0,
            $formNode->count(),
            'Password form element should exist.',
        );
        ...
        $this->assertNotEmpty(
            $errors,
            'Expected at least one validation error for mismatched passwords.',
        );
        $this->assertTrue(
    ...
    public function testPasswordFormChangesPasswordAndRedirects(): void
    {
        ...
        if ($status < 200 || $status >= 300) {
            ...
            @fwrite(
                \STDERR,
                '[MemberPasswordFormTest] GET /en/profile/password (change) status='.
                    $status.
                    ' Location='.
                    $loc.
                    \PHP_EOL,
        ...
        $this->assertTrue(
            ...
            'Successful password change should redirect (got '.$status.').',
        );

        $loc = $this->client->getResponse()->headers->get('Location');
        $this->assertNotEmpty(
            $loc,
            'Redirect location header missing after password change.',
        );
        $this->client->request('GET', $loc);
        $this->assertSame(
            200,
            $this->client->getResponse()->getStatusCode(),
            'Redirect target should load (profile page).',
        );
        ...
        $this->assertFalse(
            $isAuthed(),
            'Old password should no longer authenticate the user (expected failure).',
        );
        ...
        $this->assertTrue(
            $isAuthed(),
            'User should be authenticated with the new password.',
        );
    }
