...
// Read JSON config from data-config and apply known overrides
(function () {
    var raw = c.getAttribute("data-config");
    if (raw && typeof raw === "string") {
        try {
            var cfg = JSON.parse(raw);
            if (cfg && typeof cfg === "object") {
                if (
                    typeof cfg.gridSize === "number" &&
                    isFinite(cfg.gridSize) &&
                    cfg.gridSize > 2
                ) {
                    gnum = Math.floor(cfg.gridSize);
                    w = _x / gnum;
                    h = _y / gnum;
                }
                if (cfg.spring && typeof cfg.spring === "object") {
                    if (typeof cfg.spring.p1 === "number") P1 = cfg.spring.p1;
                    if (typeof cfg.spring.p2 === "number") P2 = cfg.spring.p2;
                    if (typeof cfg.spring.n === "number") n = cfg.spring.n;
                    if (typeof cfg.spring.nVel === "number")
                        n_vel = cfg.spring.nVel;
                }
                if (typeof cfg.colorCycle === "number") {
                    // adjust hue change speed; default is 0.5 per frame
                    var speed = cfg.colorCycle;
                    if (isFinite(speed)) {
                        // use negative to mirror existing decrement
                        window.__gridHueStep = -Math.abs(speed);
                    }
                }
                if (cfg.interactive === false) {
                    window.__gridInteractive = false;
                }
            }
        } catch (e) {
            // ignore invalid JSON
...
if (window.__gridInteractive !== false) {
