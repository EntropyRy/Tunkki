...
const canvas = document.getElementById("bunka");
...
const config = {
  particleCount: 1100, // Number of particles
  particleBaseSpeed: 0.3, // Base speed of particles
  particleSpeedVariation: 1.5, // Speed variation
  ...
  fadeAmount: 0.001, // Fade intensity on each frame (lower = longer trails)
  flowFieldIntensity: 0.8, // Flow field force intensity
  noiseScale: 0.003, // Scale of the noise (smaller = smoother)
  noiseSpeed: 0.0001, // Speed of flow field change
  ...
  hueShiftRange: 120, // Range of hue shift for 'analogous' color mode
};
  ...
  switch (config.colorMode) {
    ...
    case "complement":
      ...
      const blendedR = Math.floor(
        config.particleColor.r * (1 - compRatio) + complementR * compRatio,
      );
      const blendedG = Math.floor(
        config.particleColor.g * (1 - compRatio) + complementG * compRatio,
      );
      const blendedB = Math.floor(
        config.particleColor.b * (1 - compRatio) + complementB * compRatio,
      );
...
function drawFlowField() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (config.enableTrails && p.prevX !== undefined && p.prevY !== undefined) {
...
function addColorModeSelector(controls, config) {
  ...
  colorModeSelect.addEventListener("change", (e) => {
    ...
    const hueShiftControls = document.querySelectorAll(".hue-shift-control");
  ...
  toggleButton.addEventListener("click", () => {
    ...
    if (controlsVisible) {
      ...
      elements.forEach((el) => {
        ...
        if (el.classList && el.classList.contains("hue-shift-control")) {
