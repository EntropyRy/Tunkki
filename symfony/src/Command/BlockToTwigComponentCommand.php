...
class BlockToTwigComponentCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this->addArgument(
            ...
            'Block class name (e.g., BookingsBlock)',
        )
            ->addOption(
                'component-name',
                'c',
                InputOption::VALUE_OPTIONAL,
                'Custom component name (defaults to block name without "Block" suffix)',
            )
            ->addOption(
                'dry-run',
                null,
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_NONE,
                'Keep original block files (do not remove)',
            );
    }
    ...
    protected function execute(
        ...
        $componentName =
            $input->getOption('component-name') ??
            $this->deriveComponentName($blockName);
        ...
        $componentClassContent = $this->generateComponentClass(
            $componentName,
            $blockInfo,
        );
        $componentPath =
            $this->projectDir.self::COMPONENT_DIR.$componentName.'.php';
        ...
        if ($blockInfo['template']) {
            $blockTemplatePath =
                $this->projectDir.
                self::BLOCK_TEMPLATE_DIR.
                basename($blockInfo['template']);
            if (file_exists($blockTemplatePath)) {
                ...
                if (false !== $templateContent) {
                    $componentTemplateContent = $this->convertTemplate(
                        $templateContent,
                    );
                    $componentTemplatePath =
                        $this->projectDir.
                        self::COMPONENT_TEMPLATE_DIR.
                        $this->getTemplateFileName($componentName);

                    if ($dryRun) {
                        $io->writeln(
                            '<comment>[DRY RUN]</comment> Would create template: '.
                                $componentTemplatePath,
                        );
                    } else {
                        ...
                        $io->success(
                            'Template created: '.$componentTemplatePath,
                        );
                    }
        ...
        if ($dryRun) {
            ...
            $io->writeln('');
            $io->writeln(
                '<comment>[DRY RUN]</comment> Would create: '.$componentPath,
            );
        } else {
        ...
        $io->listing([
            ...
            $keepBlock
                ? 'Original block kept (use --keep-block to remove)'
                : 'Remove block service registration from config/services.yaml',
            'Remove old block template from '.
            self::BLOCK_TEMPLATE_DIR.
            ' if no longer needed',
        ]);
        ...
        if (!$keepBlock && !$dryRun) {
            $io->warning(
                'Remember to remove block registration from config/services.yaml',
            );
            $io->note(
                \sprintf(
                    'Search for: %s or %s',
                    $blockName,
                    strtolower(str_replace('Block', '', $blockName)),
                ),
            );
        }
    ...
    private function parseBlockClass(string $content): array
    {
        ...
        if (
            ...
            preg_match_all(
                '/(?:protected|private|public)\s+(?:readonly\s+)?([^\s]+)\s+\$([^,)]+)/',
                $constructorMatch[0],
                $paramMatches,
            );
        ...
        if (
            ...
            if ([] !== $settingMatches[1]) {
                $counter = \count($settingMatches[1]);
                for ($i = 0; $i < $counter; ++$i) {
        ...
        if (
            preg_match(
                "/public function execute\([^{]*\)\s*:\s*Response\s*{([^}]+(?:{[^}]*}[^}]*)*)}/",
                $content,
                $executeMatch,
            )
        ) {
        ...
        preg_match_all(
            "/(?:public|protected|private)\s+function\s+([a-zA-Z_]\w*)\s*\([^)]*\)[^{]*{/",
            $content,
            $methodMatches,
        );
    ...
    private function generateComponentClass(
        ...
        $mountMethod = '';
        $mountSignature =
            [] !== $mountParams ? implode(', ', $mountParams) : '';

        if (!empty($blockInfo['methods']['execute'])) {
            $executeBody = $this->convertExecuteToMount(
                $blockInfo['methods']['execute'],
            );
            $assignmentsBlock =
                [] !== $mountAssignments
                    ? "\n        ".
    ...
    private function formatDefaultValue(string $value, string $phpType): string
    {
        ...
        if ('bool' === $phpType || 'int' === $phpType || 'float' === $phpType) {
            return $trimmed;
        }
    ...
    private function findFullTypeFromUses(
        string $shortType,
        array $uses,
    ): ?string {
        foreach ($uses as $use) {
    ...
    private function convertExecuteToMount(string $executeBody): string
    {
        ...
        $cleaned = preg_replace(
            ...
            '// TODO: Remove block context usage',
            (string) $cleaned,
        );
        $cleaned = preg_replace(
            '/\$blockContext->getTemplate\(\)/',
            '// TODO: Remove template reference',
            (string) $cleaned,
        );
    ...
    private function convertTemplate(string $content): string
    {
        ...
        $converted = preg_replace(
            "/{%\s*extends\s+sonata_block\.templates\.block_base\s*%}/",
            '',
            $content,
        );
        ...
        $converted = preg_replace(
            "/{%\s*for\s+([a-zA-Z_]\w*)\s+in\s+([a-zA-Z_]\w*)\s*%}/",
            '{% for $1 in this.$2 %}',
            (string) $converted,
        );
