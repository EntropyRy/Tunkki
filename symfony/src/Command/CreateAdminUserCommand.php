...
final class CreateAdminUserCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument(
                ...
                'Email for the admin account (unique)',
            )
            ...
            ->addOption(
                ...
                'Add ROLE_SUPER_ADMIN in addition to ROLE_ADMIN',
            )
            ->addOption(
                'role',
                null,
                InputOption::VALUE_IS_ARRAY | InputOption::VALUE_REQUIRED,
                'Additional roles, can be specified multiple times',
                [],
            )
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_REQUIRED,
                'Password (non-interactive). Use with care.',
            )
            ->addOption(
                'ask-password',
                null,
                InputOption::VALUE_NONE,
                'Prompt for password interactively (hidden)',
            )
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_NONE,
                'If user exists, update roles and password if provided',
            );
    }
    ...
    protected function execute(
        ...
        if ($rawPassword && $askPassword) {
            $io->warning(
                'Both --password and --ask-password provided. Using --password value.',
            );
            $askPassword = false;
        }
        ...
        /** @var EntityRepository<Member> $memberRepo */
        ...
        if ($isNewUser && !$rawPassword) {
            $io->error(
                'Password is required when creating a new user. Provide --password or --ask-password.',
            );
        ...
        if (null === $member) {
            ...
            if (null === $member->isEmailVerified()) {
                $member->setEmailVerified(true);
            }
            if (null === $member->isAllowInfoMails()) {
                $member->setAllowInfoMails(true);
            }
            if (null === $member->isAllowActiveMemberMails()) {
                $member->setAllowActiveMemberMails(true);
            }
        }
        ...
        if (null === $user) {
            ...
            if (!$updateIfExists) {
                $io->warning(
                    \sprintf(
                        'User for %s already exists. Use --update-if-exists to update roles/password.',
                        $email,
                    ),
                );
            }
        ...
        if ($rawPassword && ($isNewUser || $updateIfExists)) {
            ...
            $user->setPassword(
                $this->hasher->hashPassword($user, (string) $rawPassword),
            );
        }
